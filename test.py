...
for imgx in onlyfiles:
    img = cv2.imread(join(mypath,imgx))
    ...
    img = cv2.drawContours(img, newcontour, -1, (0, 255, 0), 3)


    # with open('shapes.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    #     pickle.dump([newcontour[1], newcontour[2], newcontour[3]], f)
    cv2.namedWindow(imgx, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(imgx, 1200,1200)
    cv2.imshow(imgx, img)
    # print(hierarchy)
